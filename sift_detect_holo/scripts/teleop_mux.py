...
def joy_cb(value):
    global mux_proxy, camera_mux_proxy, joystick_topic, camera_joystick_topic, joystick_button
    global servo_topic, camera_servo_topic, servo_button, selected, last_joy
...
def init():
    ...
    rospy.wait_for_service('/camera_mux/select')
    mux_proxy = rospy.ServiceProxy('/mux/select', MuxSelect)
    camera_mux_proxy = rospy.ServiceProxy('/camera_mux/select', MuxSelect)
    ...
    camera_joystick_topic = rospy.get_param("~camera_joystick_topic",camera_joystick_topic)
    servo_button = rospy.get_param("~servo_button",servo_button)
    servo_topic = rospy.get_param("~servo_topic",servo_topic)
    camera_servo_topic = rospy.get_param("~camera_servo_topic",camera_servo_topic)
    timeout = False
